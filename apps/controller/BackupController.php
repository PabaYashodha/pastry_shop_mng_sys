    ...
    switch ($status) {
        case "makeBackup":
            ...
            if (fclose($handle)) {
                ...
                $cusEmail  = 'pabayashodha@gmail.com';
        ...
        case 'getBackupData':
            $getBackupData = $backupObj->getBackupData();
            $backupArray = array();
            while ($row = $getBackupData->fetch_assoc()) {
                array_push($backupArray, $row);
            }
            echo json_encode($backupArray);
            break;


        case 'deleteBackup':
            $backupId = $_POST['backupId'];
            $deleteBackup = $backupObj->deleteBackup($backupId);
            if ($deleteBackup == 1) {
                $res=1;
                $getBackupData = $backupObj->getBackupData();
                $backupArray = array();
