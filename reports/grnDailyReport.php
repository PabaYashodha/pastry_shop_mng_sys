...
class PDF extends FPDF
{
    ...
    {
        // Logo
        $this->Image('../images/download.png', 10, 6, 20);
        // Arial bold 15
        $this->SetFont('Arial', 'B', 15);
        // Move to the right
        $this->Cell(80);
        // Title
        $this->Cell(30, 10, 'GRN Daily Report', 0, 1, 'C');
...
// Cell(float w [, float h [, string txt [, mixed border [, int ln [, string align [, boolean fill [, mixed link]]]]]]])
    ...
    function TableBody()
    {
        ...
        $grnDailyReport = $reportDb->grnDailyReport();
        //var_dump( $grnDailyReport);
        $this->SetFont('Arial', '', 8);
        $count = 1;
        while ($row = $grnDailyReport->fetch_assoc()) {
